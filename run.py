...
def train(config):
    run = wandb.init(project=config['logging']['project'], config=config, name=config['logging']['exp_name'], reinit=True, monitor_gym = True, mode=config['wandb'])
    ...
    envs = gym.wrappers.RecordEpisodeStatistics(envs, deque_size=50)
